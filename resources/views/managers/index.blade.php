        ...
        <div class="row">
                ...
                <div class="card">
                    <div class="card-header bg-white">
                        <div class="d-flex justify-content-between align-items-center">
                            ...
                            <div class="float-end">
                                @can('manager-create')
                                    <a class="btn text-dark" href="{{ route('managers.create') }}"><i class="fa fa-plus"></i> Create New Manager</a>
                                @endcan
                            </div>
                        </div>
                    </div>
            ...
            <div class="col-md-12">
                 <div class="float-start my-3">
                    <h2>Manager ({{ $total_manager }})</h2>
                </div>
                <table class="table table-bordered">
                    <tr class="bg-white">
                        <th>Sr#</th>
                        <th><i><a class=" ml-2 text-dark"
                                href="{{ route('managers.index', array_merge(request()->query(), ['sort' => 'name', 'direction' => request('direction', 'asc') == 'asc' ? 'desc' : 'asc'])) }}">Name</a></i>
                            @if (request('sort') === 'name')
                                <i class="fa {{ $direction == 'asc' ? 'fa-arrow-down' : 'fa-arrow-up' }} px-2 py-2"></i>
                            @endif
                        </th>
                        <th><a class=" ml-2 text-dark"
                                href="{{ route('managers.index', array_merge(request()->query(), ['sort' => 'email', 'direction' => request('direction', 'asc') == 'asc' ? 'desc' : 'asc'])) }}">Email</a></i>
                            ...
                            <tr>
                                ...
                                <td>
                                    <form id="deleteForm{{ $manager->id }}"
                                        action="{{ route('managers.destroy', $manager->id) }}" method="POST">
                                        <a class="btn text-dark" href="{{ route('managers.show', $manager->id) }}"><i class="fa fa-eye"></i></a>
                                        @can('manager-edit')
                                            <a class="btn text-dark"
                                                href="{{ route('managers.edit', $manager->id) }}"><i class="fa fa-edit"></i></a>
                                        @endcan
                                        @csrf
                                        @method('DELETE')
                                        @can('manager-delete')
                                            <button type="button" onclick="confirmDelete('{{ $manager->id }}')"
